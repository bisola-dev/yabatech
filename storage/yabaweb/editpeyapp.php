...
if (isset($_POST['submit'])){
   $candid = mysqli_real_escape_string($conn, $_POST['candid']);}
      ...
      //echo $cdate.''.$Appfee.' '.$currentsession.''.$candidate;

   
   if( $cdate == "" || $appfee == ""||$csession=="") {
  $reportalert="<script type ='text/javascript'>
  alert('please do not submit empty form.')
  window.location.href='editpeyapp.php';
  </script>";
   

 }
      
   else if(strlen($appfee) >20){
            $reportalert='<script type="text/javascript">
        alert(" The specified amount is incorrect.");
        window.location.href="editpeyapp.php";
        </script>';
     ...
     $kolo = " UPDATE app SET closindate ='$cdate', fee='$appfee', csesion='$csession' WHERE id = $candid";

     if($csession==1){
    $oga = mysqli_query($conn, " UPDATE app SET csesion = 0 WHERE id != $candid");}
...
if($candid==""){
        header("Location:peyapp.php");

}

$kingpaul = mysqli_query($conn, "SELECT * FROM app WHERE id=$candid");
 while ($row=mysqli_fetch_assoc($kingpaul)) {
      $sesion1 = $row['sesion'];
      $closindate1 =$row['closindate'];
      $fee1= $row['fee'];
      $csesion1 = $row['csesion'];
         ...
         <div class="content-wrapper">
            ...
            <section class="content-header">
               ...
               <div class="header-title">
                  <h1> Edit Application  <?php echo $sesion1;?> Fee Panel</h1>
                  <small></small>
               </div>
            </section>
            <!-- Main content -->
                        ...
                        <div class="panel-body">
                           <form class="col-sm-6" method="post" action="">
                              ...
                              <div class="form-group">
                                 ...
                                 <input type="date" name="cdate" class="form-control" value="<?php echo $closindate1;?>" required>

                                 <input type="hidden" name="candid" class="form-control" value="<?php echo $candid;?>" >
                              </div>
                              
                              <div class="form-group">
                                 <label>Application Fee</label>
                                 <input type="number" name="appfee" class="form-control" value="<?php echo $fee1;?>"  required>
                              </div>

...
<head>
   ...
   <body class="hold-transition sidebar-mini">
    ...
    <!--preloader-->
            ...
            <section class="content-header">
               ...
               <div class="header-title">
                  <h1> @foreach($skooldeet  as $data)
                    <b> <h4> welcome  to "{{$data->skolssn}}"</h4><b>
             @endforeach</h1>
               </div>
            </section>
         	<!-- Page Wrapper -->
                     ...
                     <div class="panel panel-bd lobidrag">
                        <div class="panel-heading">
                           <div class="btn-group" id="buttonexport">
                              <a href="#">
                                @foreach($deppt as $data)
                                 <h4> ADD further details  to  department "{{$data->deptnam}}"</h4>
                                 @endforeach
                              </a>
                           </div>
                           <button> <a href="{{url('/managesch')}}">Back </a></button>
                        </div>
                        <div class="panel-body">
                       
                        <form method="post"  action='/devdeptproc/{id}' enctype="multipart/form-data">
                           @csrf	 
                        @foreach($deppt as $datda)    

                  
                            <div class="form-group ">
                              <label>Department Name</label>
                              <input type="text"  class="form-control" name="deptnam" value="{{$datda->deptnam}}" > 
                              <input type="hidden"  class="form-control" name="id" value="{{$datda->id}}" >
                            </div> 
                         

                            <div class="form-group ">
                            <label>Department Acronym </label>
                            <input type="text" class="form-control"  name="depatkro" value="{{$datda->depatkro}}" >
                            </div>
                            ...
                            <input type="text" class="form-control"   name="depatitl" value="{{$datda->depatitl}}" >
                            </div>

                            <div class="form-group ">
                              <label>About department</label>
                              <textarea class="form-control" id="body" name="abtdept" >{!!$datda->abtdept!!}</textarea>
                            </div>


                            <div class="form-group ">
                              <label>History of Department</label>
                              <textarea class="form-control" id="body2"name="hisdept" >{!!$datda->hisdept!!}</textarea>
                            </div>
                            @endforeach
                        ...
                        @foreach($deppt as $datda)  
                        <div class="form-group ">  
                              <input type="hidden"  class="form-control" name="skolid" value="{{$datda->skolid}}"> 
                              <input type="hidden"  class="form-control" name="deptid" value="{{$datda->id}}" >
                            @endforeach
                            </div>

                            <div class="form-group ">
                            <label>Select staff of the department</label>
                            <select class="form-control"  name="fullnam"  >
                          <option value="">-- Select staff of department --</option>
                          @foreach($staff as $staff)
                         <option value="{{$staff->fulln}}">{{$staff->fulln}}</option>
                         @endforeach
                           </select>
                            </div>
                              ...
                              <table id="dataTableExample1" class="table table-bordered table-striped table-hover">
                                 <thead>
                                    <tr class="info">
                                       ...
                                       <th>Upload department images<th>
                                    </tr>
                                 </thead>
                                 <tbody>                             
                            
                                    <tr>
                                    @forelse($deppt as $datda)
                                       ...
                                       <td style="display: none;">{{$datda->id}}</td>
                                            ...
                                            <td class="text-right">
                                              <div class="dropdown dropdown-action">
                                                    <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="false"><i class="fa fa-upload"></i></a>
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <a class="dropdown-item ria" href="#" data-toggle="modal" data-target=""><i class="fa fa-upload m-r-5"></i> Upload hod's image</a><br>
                                                        <a class="dropdown-item lumi" href="#" data-toggle="modal" data-target=""><i class="fa fa-upload m-r-5"></i> Upload Unit image</a>
                                                      </div>
                                                     </div> 
                                                     </td>     
                                        <td>
                                        ...
                                        <a href= "{{url('/addedstaff/'.$datda->skolid.'/'.$datda->id)}}" data-target=""><i class="fa fa-pencil m-r-5"></i> View staff</a>
                                        @endforeach  
                                       </td>
                                            </div>
                                 @empty
                         <p><b>No records yet</b></p>
                           @endforelse  
                              </table>            
                           </div>
                        </div>
     ...
     <div id="truce" class="modal custom-modal fade" role="dialog">
                     ...
                     <div class="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3">
                        ...
                        @foreach($deppt as $data)  
							<td><img src="/storage/hodimg/{{($data->hodimg)}}" width="100px", height="100px"></td>
								</div>
								</div>
                       
                           <h5 class="modal-title">Upload Hod's Image</h5>
                        ...
                        <div class="panel-body">
                         ...
                         <form method="post" action='/hodimg/{id}'  enctype="multipart/form-data">
                                  @csrf	

                                  <div class="col-sm-6 col-md-12">
                                 <div class="form-group"> 
              
                       <label>Select Hod's logo (png, jpg or jpeg format allowed) <span class="text-danger">*</span></label>
                       <input type="hidden" name="id"  value="{{$datda->id}}" > @endforeach
                      <input type="file" class="form-control" name="hodimg">
                     </div>
                     </div>
      ...
      <div id="realq" class="modal custom-modal fade" role="dialog">
       <div class="modal-dialog modal-dialog-centered" role="document">
       <div class="modal-content">
          <div class="modal-header">
          @foreach($deppt as $data) 
          <div class="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3">
							<div class="profile-widget"> 
								<div class="profile-img">
                        <td><img src="/storage/unitimg/{{($data->unitimg)}}" width="100px", height="100px"></td>
								</div>
								</div>
                   
                        <h5 class="modal-title">Upload Unit Image</h5>
                 ...
                 <div class="panel-body">
                  ...
                  <form method="post" action='/unitimg/{id}'  enctype="multipart/form-data">
                     @csrf	
                     <div class="col-sm-6 col-md-12">
              <div class="form-group"> 

             <label>Select Unit logo (png, jpg or jpeg format allowed) <span class="text-danger">*</span></label>
            <input type="hidden" name="id" value="{{$datda->id}}" >     @endforeach
                          
             <input type="file" class="form-control" name="unitimg">
              </div>
                 </div>
     ...
     <a href="javascript:void(0);" data-dismiss="modal" class="btn btn-danger cancel-sm">Cancel</a>
         ...
         <footer class="main-footer">
            <strong>Copyright &copy; 2023-2026 <a href="#">thememinister</a>.</strong> All rights reserved.
         </footer>
      </div>
      <!-- ./wrapper -->
      <!-- Start Core Plugins
